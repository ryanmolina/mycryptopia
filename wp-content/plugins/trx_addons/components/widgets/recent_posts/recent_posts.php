...
//			'suppress_filters' => true
			...
			foreach ($recent_posts as $post) {
				...
				trx_addons_get_template_part('templates/tpl.posts-list.php',
											...
											apply_filters('trx_addons_filter_widget_posts_args', array(
															'counters' => 'comments',	// 'comments' 'views'
		...
		if (!empty($output)) {
			trx_addons_get_template_part(TRX_ADDONS_PLUGIN_WIDGETS . 'recent_posts/tpl.default.php',
											...
											apply_filters('trx_addons_filter_widget_args',
												array_merge($args, compact('title', 'output', 'link', 'link_title')),
											$instance, 'trx_addons_widget_recent_posts')
										);
		}
	}
	...
	function form($instance) {
		...
		$this->show_field(array('name' => 'link_title',
...
if ( !function_exists( 'trx_addons_sc_widget_recent_posts' ) ) {
	function trx_addons_sc_widget_recent_posts($atts, $content=null){
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			...
			if ($atts['link_title']!='' && $atts['link']!='') {
				$output .= '<div class="go_link"><a class="sc_button sc_button_bordered sc_button_size_large" href="'.esc_url($atts['link']).'">'.esc_html($atts['link_title']).'</a></div>';
			}
