...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
																...
																'hover' => 'zoomin',
																'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'avatar' : 'big'), 'currency-default'),
																),
															'currency-default'
															)
										);
	...
	<div class="sc_currency_item_info">
		<div class="sc_currency_item_header">
			<h4 class="sc_currency_item_title"><a href="<?php echo esc_url($link); ?>"><?php
				the_title();
			?></a></h4>

			<?php if (!empty($meta['price'])) {
			?><div class="currency_price"><?php echo do_shortcode($meta['price']); ?></div><?php
			}

			if (!empty($meta['info_text'])) { ?>

			<div class="sc_currency_item_subtitle"><?php trx_addons_show_layout($meta['info_text']);?></div>

			<?php } ?>
		</div>
		<div class="sc_currency_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button sc_button_default"><?php esc_html_e('information', 'trx_addons'); ?></a></div>
	</div>
</div>
<?php
