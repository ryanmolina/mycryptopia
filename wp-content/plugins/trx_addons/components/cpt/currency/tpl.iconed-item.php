<?php
/**
 * The style "iconed" of the currency item
 *
 * @package WordPress
...
<div class="sc_currency_item<?php
	...
	echo ' sc_currency_item_featured_'.esc_attr($args['featured_position']);
?>"<?php
	if (!empty($args['popup'])) {
...
?>>
	<div class="sc_currency_item_header"<?php if (!empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		<?php
		...
		if (isset($args['hide_excerpt']) && $args['hide_excerpt']==1) {
			?>
			<h6 class="sc_currency_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h6>
	...
	<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
		<div class="sc_currency_item_content">
			<h6 class="sc_currency_item_title<?php if (!empty($meta['price'])) echo ' with_price'; ?>"><a href="<?php echo esc_url($link); ?>"><?php
				the_title();
				// Price
				if (!empty($meta['price'])) {
			...
			?></a></h6>
			<div class="sc_currency_item_subtitle"><?php trx_addons_show_layout(trx_addons_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_CURRENCY_TAXONOMY));?></div>
			<div class="sc_currency_item_text"><?php the_excerpt(); ?></div>
