<?php
/**
 * The style "timeline" of the currency
 *
 * @package WordPress
...
?>><?php
	...
	// Timeline
	?><div class="sc_currency_item_timeline"></div><?php

	// Featured image or icon
	if ($args['featured'] != 'none') {
		if ( has_post_thumbnail() && $args['featured']=='image') {
			?><div class="sc_currency_item_marker sc_currency_item_thumb" style="background-image: url(<?php
			...
			?><a href="<?php echo esc_url($link); ?>"
				 ...
				 class="sc_currency_item_marker sc_currency_item_icon <?php
						echo !empty($svg) 
								? 'sc_icon_type_svg'
								: (!empty($img) 
									? 'sc_icon_type_img'
			...
			?><span class="sc_currency_item_marker sc_currency_item_number"><?php
				printf("%02d", $number);
			?></span><?php
		}
	}
	...
	<div class="sc_currency_item_info">
		...
		if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']!='image')) {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
															'class' => 'sc_currency_item_featured',
															...
															'post_info' => !empty($meta['price']) 
																				? '<span class="sc_currency_item_price">'.esc_html($meta['price']).'</span>'
																				: ''
															),
															'currency-timeline'
															)
		...
		<div class="sc_currency_item_header">
			...
			if ($args['featured']=='number' && $args['featured_position']=='top') {
				?><div class="sc_currency_item_subtitle"><?php trx_addons_show_layout(trx_addons_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_CURRENCY_TAXONOMY));?></div><?php
			}
			?>
			<<?php echo esc_html($title_tag); ?> class="sc_currency_item_title<?php if (!$price_showed && !empty($meta['price'])) echo ' with_price'; ?>"><a href="<?php echo esc_url($link); ?>"><?php
				the_title();
				// Price
				if (!$price_showed && !empty($meta['price'])) {
					?><div class="sc_currency_item_price"><?php echo esc_html($meta['price']); ?></div><?php
				}
