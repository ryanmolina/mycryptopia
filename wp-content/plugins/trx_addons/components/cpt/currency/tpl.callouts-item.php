<?php
/**
 * The style "default" of the currency
 *
 * @package WordPress
...
$args = get_query_var('trx_addons_args_sc_currency');
$number = get_query_var('trx_addons_args_item_number');

$meta = get_post_meta(get_the_ID(), 'trx_addons_options', true);
$link = get_permalink();
if (empty($args['id'])) $args['id'] = 'sc_currency_'.str_replace('.', '', mt_rand());
...
?>
<div class="sc_currency_item<?php
	...
	echo $args['featured']=='image' 
					...
					: ($args['featured']=='icon' 
						...
						: ($args['featured']=='number' 
							? ' with_number' 
							: ''));
	echo ' sc_currency_item_featured_'.esc_attr($args['featured']!='none' ? $args['featured_position'] : 'none');
?>"<?php
	if (!empty($args['popup'])) {
		?> data-post_id="<?php echo esc_attr(get_the_ID()); ?>"<?php
		?> data-post_type="<?php echo esc_attr(TRX_ADDONS_CPT_CURRENCY_PT); ?>"<?php
	}
?>>
	<?php
	// Featured image
	if ($args['featured'] != 'none') {
		?><div class="sc_currency_item_marker_bg"></div><?php
		if ( has_post_thumbnail() && $args['featured']=='image') {
			?><div class="sc_currency_item_thumb sc_currency_item_marker" style="background-image: url(<?php
				...
				?>);"></div><?php
			?><div class="sc_currency_item_thumb sc_currency_item_marker sc_currency_item_marker_back" style="background-image: url(<?php
				$trx_addons_attachment_src = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'masonry' );
				if (!empty($trx_addons_attachment_src[0]))
					echo esc_url($trx_addons_attachment_src[0]);
				?>);"></div><?php
		...
		// Icon
			...
			?><a href="<?php echo esc_url($link); ?>"
				 ...
				 class="sc_currency_item_icon sc_currency_item_marker <?php
						echo !empty($svg) 
								? 'sc_icon_type_svg'
								: (!empty($img) 
									? 'sc_icon_type_img'
			...
			?><a href="<?php echo esc_url($link); ?>"
				 ...
				 class="sc_currency_item_icon sc_currency_item_marker sc_currency_item_marker_back <?php
						echo !empty($svg) 
								? 'sc_icon_type_svg'
								: (!empty($img) 
									? 'sc_icon_type_img'
		...
		// Number
			...
			?><span class="sc_currency_item_number sc_currency_item_marker"><?php
				printf("%02d", $number);
			?></span><?php
			?><span class="sc_currency_item_number sc_currency_item_marker sc_currency_item_marker_back"><?php
				printf("%02d", $number);
			?></span><?php
		}
	}
	?>	
	<div class="sc_currency_item_info">
		<div class="sc_currency_item_header">
			<h4 class="sc_currency_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h4>
			<div class="sc_currency_item_subtitle"><?php trx_addons_show_layout(trx_addons_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_CURRENCY_TAXONOMY));?></div>
		</div>
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			<div class="sc_currency_item_content"><?php the_excerpt(); ?></div>
			<div class="sc_currency_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_simple', 'sc_currency', $args)); ?>"><?php esc_html_e('Learn more', 'trx_addons'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
