<?php
/**
 * The style "tabs_simple" of the currency
 *
 * @package WordPress
...
if ($query->found_posts > 0) {
	...
	?><div class="sc_currency sc_currency_<?php
			echo esc_attr($args['type']);
			if (!empty($args['class'])) echo ' '.esc_attr($args['class']); 
			?>"><?php

		trx_addons_sc_show_titles('sc_currency', $args);
		
		?><div class="sc_currency_content sc_item_content"><?php
			...
			while ( $query->have_posts() ) { $query->the_post();
				...
				$tabs_item = '<div class="sc_currency_item sc_currency_tabs_simple_item sc_currency_tabs_list_item'
									. ($trx_addons_number-1 == $args['offset'] ? ' sc_currency_tabs_list_item_active' : '')
									. (empty($args['featured']) || $args['featured']=='image' 
											? ' with_image' 
											: ($args['featured']=='icon' ? ' with_icon' : ' with_number')
										)
									. ' sc_currency_item_featured_'.esc_attr($featured_position)
								. '">';
				...
				if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
					...
					trx_addons_get_template_part('templates/tpl.featured.php',
													...
													apply_filters('trx_addons_filter_args_featured', array(
																	'class' => 'sc_currency_item_thumb',
																	'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] < 3 ? 'medium' : 'small'), 'currency-tabs-simple')
																	),
																'currency-tabs-simple'
																)
												);
					...
					$tabs_item .= sprintf('<span class="sc_currency_item_number">%02d</span>', $trx_addons_number);
				}
				// Post title and subtitle
				$tabs_item .= '<div class="sc_currency_item_info">'
								. '<h6 class="sc_currency_item_title">' . get_the_title() . '</h6>'
								. '<div class="sc_currency_item_subtitle">'
									. trx_addons_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_CURRENCY_TAXONOMY, false)
								. '</div>'
							. '</div>';
				$tabs_item .= '</div>';
				// Save to the list
				$tabs_list[] = array(
									'title' => $tabs_item,
									'content' => '<div class="sc_currency_tabs_content_item'
													. ($trx_addons_number-1 == $args['offset'] ? ' sc_currency_tabs_content_item_active' : '')
													. '">'
														. '<div class="sc_currency_tabs_content_item_text">'
															. get_the_excerpt()
														. '</div>'
														. '<div class="sc_currency_item_button sc_item_button">'
															. '<a href="'.esc_url(get_permalink()).'" class="'.esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button', 'sc_currency', $args)).'">'.esc_html__('Learn more', 'trx_addons').'</a>'
														. '</div>'
													. '</div>'
									);
			}
			...
			if (count($tabs_list) > 0) {
				// Display titles
				?><div class="sc_currency_tabs_list"><?php
					foreach($tabs_list as $item) trx_addons_show_layout($item['title']);
				?></div><?php
				// Display contents
				?><div class="sc_currency_tabs_content"><?php
					foreach($tabs_list as $item) trx_addons_show_layout($item['content']);
				?></div><?php
			}
		?></div><!-- /.sc_currency_content --><?php

		trx_addons_sc_show_links('sc_currency', $args);
