<?php
/**
 * The style "hover" of the currency item
 *
 * @package WordPress
...
<div class="sc_currency_item<?php
	...
	echo $args['featured']=='icon' 
				...
				: ($args['featured']=='number' 
					? ' with_number' 
					: ' sc_currency_item_featured_none');
?>"<?php
	if (!empty($args['popup'])) {
...
?>>
	<div class="sc_currency_item_header <?php echo $args['hide_bg_image']==1 ? ' without_image' : ''; ?>"<?php if ($args['hide_bg_image']==0 && !empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
		<div class="sc_currency_item_header_inner">
			<?php
		...
		<a class="sc_currency_item_link" href="<?php echo esc_url($link); ?>"></a>
	</div>
	<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
		<div class="sc_currency_item_content"<?php if (!empty($image)) echo ' style="background-image: url('.esc_url($image).');"'; ?>>
			<div class="sc_currency_item_content_inner">
				<h6 class="sc_currency_item_title"><a href="<?php echo esc_url($link); ?>"><?php the_title(); ?></a></h6>
				<div class="sc_currency_item_subtitle"><?php trx_addons_show_layout(trx_addons_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_CURRENCY_TAXONOMY));?></div>
