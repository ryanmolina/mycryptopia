...
CCC.filterNumberFunctionPolyfill = function(value,decimals){
  ...
  var parts = numberWithCorrectDecimals.toString().split(".");   //console.log(parts);
  parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ","); // place commas in correct locations
  if( parts[1] === undefined){ parts[1] = '0'; } while( parts[1].length < decimals ) {  parts[1] += '0';  } //ensure filled out decimal places
  return parts.join(".");
}
...
CCC.convertValueToDisplay =  function(symbol,value,type,fullNumbers){
    ...
    if(symbol!=''){
      //prefix = symbol+' ';  //we are doing the currency symbol elsewhere now
    }
    if(value<0){
      valueSign = -1;
    }
