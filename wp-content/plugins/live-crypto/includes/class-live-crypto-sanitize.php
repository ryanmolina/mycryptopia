 ...
 *
 * @package    Live Crypto
 * @subpackage Live Crypto/includes
 */

class Live_Crypto_Sanitize {
	...
	public function clean() {
		 ...
		 */
		do_action( 'happyrobotstudio_pre_sanitize', $sanitized );
		...
		switch ( $this->type ) {
			...
			case 'numberdecimal'	: $sanitized = sanitize_text_field( $this->data ); break;
			...
			case 'colorselect'		: $sanitized = sanitize_text_field( $this->data ); break;
		 ...
		 */
		do_action( 'happyrobotstudio_post_sanitize', $sanitized );
	...
	public function set_type( $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'live-crypto' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'live-crypto' ) );
