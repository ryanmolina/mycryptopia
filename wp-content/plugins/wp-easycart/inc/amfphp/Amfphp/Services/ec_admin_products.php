...
//		$GLOBALS['ec_roleprices'] = new ec_roleprices( );
		...
		if( $post_check ){
			...
			$this->db->query( $this->db->prepare( "UPDATE " . $this->db->prefix . "posts SET post_content = %s, post_status = %s, post_title = %s, post_name = %s, guid = %s, post_excerpt = %s WHERE ID = %d", "[ec_store modelnumber=\"" . $product_row->model_number . "\"]", $status, $GLOBALS['language']->convert_text( $title ), $post_slug, $guid, $product_row->description, $product_row->post_id ) );
		}else{
			/* Manually Insert Post */
