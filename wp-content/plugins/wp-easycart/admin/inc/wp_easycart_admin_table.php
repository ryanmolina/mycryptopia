...
final class wp_easycart_admin_table{
		...
		if( count( $this->get_vars ) ){
			for( $i=0; $i<count( $this->get_vars ); $i++ ){
				if( isset( $_GET[$this->get_vars[$i]] ) )
					echo '<input type="hidden" name="' . $this->get_vars[$i] . '" value="' . htmlspecialchars( $_GET[$this->get_vars[$i]], ENT_QUOTES ) . '" />';
			}
		}
	...
	private function print_paging( $show_pagenum_box = true ){
		if( $this->record_count > $this->showing ){
			...
			if( $show_pagenum_box ){
				echo '<input class="current-page" type="text" name="pagenum" value="' . $this->current_page . '" size="1">';
			}else{
	...
	private function print_table_header( ){
		...
		echo '<th width="' . ( $actions_width[count( $this->actions )] ) . '"></th>';
		echo '</thead>';
	}
