...
class wp_easycart_admin_details{
	...
	protected function print_date_field( $column ){
		...
		else if ( is_numeric($column['value']) && (int)$column['value'] == $column['value'] ) $print_date = date( get_option( 'date_format' ),$column['value']);
		else if ( !is_numeric($column['value']) || (int)$column['value'] != $column['value'] ) $print_date = date( get_option( 'date_format' ), strtotime( $column['value'] ) );
