...
class wp_easycart_admin_details_products extends wp_easycart_admin_details{
	...
	protected function init_data( ){
		...
		$this->advanced_options = $this->wpdb->get_results( $this->wpdb->prepare( "SELECT ec_option.*, ec_option_to_product.option_to_product_id FROM ec_option_to_product, ec_option WHERE ec_option_to_product.product_id = %d AND ec_option.option_id = ec_option_to_product.option_id", $this->id ) );
	...
	public function print_advanced_options_field( $column ){
		...
		echo '<select name="add_new_advanced_option" id="add_new_advanced_option">';
		...
		echo '<div id="price_tiers_holder">';
			if( count( $this->price_tiers ) ){
				foreach( $this->price_tiers as $price_tier ){
					echo '<div class="ec_admin_price_tier_row" id="ec_admin_product_details_price_tier_row_' . $price_tier->pricetier_id . '"><span>' . $price_tier->quantity . '</span><span>' . $GLOBALS['currency']->get_currency_display( $price_tier->price ) . '</span><span><a href="" onclick="return ' . $delete_new_click_action . '( \'' . $price_tier->pricetier_id . '\' );" title="Delete"><div class="dashicons-before dashicons-trash"></div></a></span></div>';
				}
			}else{
