...
final class wp_easycart_admin_products{
	...
	public function duplicate_product( ){
		...
		$wpdb->query( $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "posts( post_content, post_status, post_title, post_name, guid, post_type ) VALUES( %s, %s, %s, %s, %s, %s )", "[ec_store modelnumber=\"" . $randmodel . "\"]", $status, $GLOBALS['language']->convert_text( $product->title ), $post_slug, $guid, "ec_store" ) );
	...
	public function save_new_quick_product( ){
		if( current_user_can( 'manage_options' ) ){
			...
			$post_slug = preg_replace( "/[^A-Za-z0-9\-]/", '', str_replace( ' ', '-', stripslashes_deep( strtolower( $title ) ) ) );
			$sku = preg_replace( "/[^A-Za-z0-9\-]/", '', str_replace( ' ', '-', stripslashes_deep( $_POST['ec_new_product_sku'] ) ) );
			...
			/* Manually Insert Post */
			$wpdb->query( $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "posts( post_content, post_status, post_title, post_name, guid, post_type, post_excerpt ) VALUES( %s, %s, %s, %s, %s, %s, %s )", "[ec_store modelnumber=\"" . $sku . "\"]", $post_status, $GLOBALS['language']->convert_text( $title ), $post_slug, $guid, "ec_store", '' ) );
			$post_id = $wpdb->insert_id;
	...
	public function save_product_details_basic( ){
		if( current_user_can( 'manage_options' ) ){
			...
			$post_slug = preg_replace( "/[^A-Za-z0-9\-]/", '', str_replace( ' ', '-', stripslashes_deep( strtolower( $_POST['post_slug'] ) ) ) );
			...
			if( $this->verify_model_number( ) ){
				if( $product_id != '0' ){
					...
					if( $post_check ){
						/* Manually Update Post */
						$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "posts SET post_content = %s, post_status = %s, post_title = %s, post_name = %s, guid = %s, post_excerpt = %s WHERE ID = %d", "[ec_store modelnumber=\"" . $model_number . "\"]", $status, $GLOBALS['language']->convert_text( $title ), $post_slug, $guid, $description, $product_row->post_id ) );
					}else{
						/* Manually Insert Post */
						$wpdb->query( $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "posts( post_content, post_status, post_title, post_name, guid, post_type, post_excerpt ) VALUES( %s, %s, %s, %s, %s, %s, %s )", "[ec_store modelnumber=\"" . $model_number . "\"]", $status, $GLOBALS['language']->convert_text( $title ), $post_slug, $guid, "ec_store", $description ) );
						$post_id = $wpdb->insert_id;
				...
				for( $i=0; $i<count( $rows ); $i++ ){
					...
					if( $rows[$i][$this->product_id_index] != 0 && $rows[$i][$this->product_id_index] != "" ){ // product_id is available
						...
						if( !in_array( $product_id, $valid_product_ids ) ){
							...
							/* Manually Update Post */
							$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "posts SET post_content = %s, post_status = %s, post_title = %s WHERE ID = %d", "[ec_store modelnumber=\"" . $rows[$i][$this->model_number_index] . "\"]", $status, $GLOBALS['language']->convert_text( $rows[$i][$this->title_index] ), $post_id ) );
						...
						if( in_array( $model_number, $existing_model_numbers ) ){
							...
							$post_slug = preg_replace( "/[^A-Za-z0-9\-]/", '', str_replace( ' ', '-', stripslashes_deep( strtolower( $rows[$i][$this->title_index] ) ) ) );
							$store_page = get_permalink( get_option( 'ec_option_storepage' ) );
							if( strstr( $store_page, '?' ) )
							...
							/* Manually Insert Post */
							$wpdb->query( $wpdb->prepare( "INSERT INTO " . $wpdb->prefix . "posts( post_content, post_status, post_title, post_name, guid, post_type ) VALUES( %s, %s, %s, %s, %s, %s )", "[ec_store modelnumber=\"" . $rows[$i][$this->model_number_index] . "\"]", $status, $GLOBALS['language']->convert_text( $rows[$i][$this->title_index] ), $post_slug, $guid, "ec_store" ) );
							$post_id = $wpdb->insert_id;
...
function ec_admin_ajax_product_details_add_price_tier( ){
	...
	echo '<div class="ec_admin_price_tier_row" id="ec_admin_product_details_price_tier_row_' . $price_tier->pricetier_id . '"><span>' . $price_tier->quantity . '</span><span>' . $price_tier->price . '</span><span><a href="" onclick="return ec_admin_product_details_delete_price_tier( \'' . $price_tier->pricetier_id . '\' );"><div class="dashicons-before dashicons-trash"></div></a></span></div>';
	die( );
}
