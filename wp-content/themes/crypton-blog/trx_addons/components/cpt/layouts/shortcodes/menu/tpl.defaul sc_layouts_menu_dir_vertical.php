...
if (!empty($trx_addons_menu)) {
	...
	if ($is_ubermenu) {
		...
		?><nav class="sc_layouts_menu sc_layouts_menu_<?php
					...
					if ($args['vertical_menu_remove_margin'] > 0) echo ' vertical_menu_no_margin';
					trx_addons_cpt_layouts_sc_add_classes($args);
					?>"<?php
				...
				if (!empty($args['css'])) echo ' style="'.esc_attr($args['css']).'"';
				?>
		><?php if (!empty($args['vertical_menu_label'])) {
				?>
					<div class="sc_layouts_menu_title"><?php echo esc_html($args['vertical_menu_label']); ?></div>
				<?php
				} 
				?><?php
			trx_addons_show_layout($trx_addons_menu);
