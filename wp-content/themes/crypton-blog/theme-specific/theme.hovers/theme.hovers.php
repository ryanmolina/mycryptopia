...
if (!function_exists('crypton_blog_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'crypton_blog_hovers_theme_setup3', 3 );
	function crypton_blog_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		crypton_blog_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'crypton-blog'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'crypton-blog') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'crypton-blog')
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'crypton-blog'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'crypton-blog') ),
				"std" => 'none',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'crypton-blog'),
...
if (!function_exists('crypton_blog_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'crypton_blog_hovers_theme_setup9', 9 );
	function crypton_blog_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'crypton_blog_hovers_frontend_scripts', 1010 );
		add_filter( 'crypton_blog_filter_localize_script','crypton_blog_hovers_localize_script' );
		add_filter( 'crypton_blog_filter_merge_scripts',	'crypton_blog_hovers_merge_scripts' );
		add_filter( 'crypton_blog_filter_merge_styles',	'crypton_blog_hovers_merge_styles' );
		add_filter( 'crypton_blog_filter_get_css', 		'crypton_blog_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'crypton_blog_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'crypton_blog_hovers_frontend_scripts', 1010 );
	function crypton_blog_hovers_frontend_scripts() {
		if ( crypton_blog_is_on(crypton_blog_get_theme_option('debug_mode')) && crypton_blog_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'crypton_blog-hovers', crypton_blog_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( crypton_blog_is_on(crypton_blog_get_theme_option('debug_mode')) && crypton_blog_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'crypton_blog-hovers',  crypton_blog_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('crypton_blog_hovers_merge_scripts')) {
	//Handler of the add_filter( 'crypton_blog_filter_merge_scripts', 'crypton_blog_hovers_merge_scripts' );
	function crypton_blog_hovers_merge_scripts($list) {
...
if (!function_exists('crypton_blog_hovers_merge_styles')) {
	//Handler of the add_filter( 'crypton_blog_filter_merge_styles', 'crypton_blog_hovers_merge_styles' );
	function crypton_blog_hovers_merge_styles($list) {
...
if (!function_exists('crypton_blog_hovers_localize_script')) {
	//Handler of the add_filter( 'crypton_blog_filter_localize_script','crypton_blog_hovers_localize_script' );
	function crypton_blog_hovers_localize_script($arr) {
		$arr['button_hover'] = crypton_blog_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('crypton_blog_hovers_add_icons') ) {
	function crypton_blog_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					crypton_blog_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'crypton-blog') : '')
										. '</a>',
										$product));
				}
				?>
...
// Add styles into CSS
if ( !function_exists( 'crypton_blog_hovers_get_css' ) ) {
	//Handler of the add_filter( 'crypton_blog_filter_get_css', 'crypton_blog_hovers_get_css', 10, 4 );
	function crypton_blog_hovers_get_css($css, $colors, $fonts, $scheme='') {
